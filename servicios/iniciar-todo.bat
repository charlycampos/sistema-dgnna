...
echo [7/10] Iniciando servicio-poi-pp117 (puerto 8007)...
start "POI-PP117 Service" cmd /k "cd servicio-poi-pp117 && iniciar.bat"
timeout /t 5 /nobreak >nul

echo [8/10] Iniciando servicio-mapa (puerto 8008)...
start "Mapa Service" cmd /k "cd servicio-mapa && iniciar.bat"
timeout /t 5 /nobreak >nul

echo [9/10] Iniciando API Gateway (puerto 8000)...
start "API Gateway" cmd /k "cd api-gateway && iniciar.bat"
timeout /t 5 /nobreak >nul

echo [10/10] Iniciando Frontend Next.js (puerto 3000)...
